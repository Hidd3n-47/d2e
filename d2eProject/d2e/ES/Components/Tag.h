...
struct Tag : public IComponent
{
    ...
    [[nodiscard]] inline std::string Serialize() const override { return SerializeUtils::Serialize(mSyncValuesOnUpdate) + SerializeUtils::Serialize(static_cast<uint8_t>(tag)); }
    inline void Deserialize(const std::string& string) override
    {
        ...
        SerializeUtils::Deserialize(value, string.substr(1));
        tag = static_cast<ComponentTag>(value);
    }
